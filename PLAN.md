...
7. **성능 목표 숫자를 두지 않는다.** 근거 없는 상한은 지켜지지도, 판단을 돕지도 않았다
...
- `--radius: 0`. 이미지만 예외로 10px
- 모든 이미지는 `.frame` 하나를 쓴다. 테두리와 모서리를 여기서만 정한다
- **이미지 배치 규칙.** 넓은 화면에서는 왼쪽, 좁은 화면에서는 위. DOM 순서를 그대로 두고
  그리드 열만 바꾼다. `order` 속성으로 시각 순서만 뒤집지 않는다
- 사진은 회색조로 처리하지 않는다. 무채색 규칙은 UI 크롬에만 적용된다
- **인물 사진은 원본 비율을 유지한다.** 자르지 않고 열 폭에 맞춘다
- **화면 스크린샷은 상단을 16:10으로 자른다.** 전체 페이지 캡처는 작게 두면 읽히지 않는다
- `width`와 `height`를 적어 자리를 미리 잡는다
- 폭은 `clamp()`로 화면을 따라간다. 고정 px이나 브레이크포인트로 끊지 않는다
- **최소 지원 폭은 320px**(iPhone SE). 이 폭에서 가로 스크롤이 생기지 않아야 한다.
  380px 미만에서는 헤더의 이름을 감추고 JS 마크만 남긴다
- 간격 스케일 `8 / 16 / 24 / 40 / 64 / 104`
- 그림자, 그라데이션, 이모지 없음
- 글은 언제나 좌측 정렬. 이미지만 가운데에 놓을 수 있다
...
- 성능 기준. 예전에 100KB·1.5s 같은 숫자를 적어 뒀는데 근거가 없어 지웠다.
  참고로 지금 First Load JS는 103KB이고 대부분 React 런타임이다.
  기준이 필요해지면 실측하고 그때 정한다
...
- 글과 프로젝트 주소는 **주제 키**로 짓는다. 제목 요약이 아니다. 제목이 바뀌어도 주소는 그대로 둔다
- 소문자·숫자·하이픈만. 형식이 틀리거나 겹치면 `content/slug.ts`가 빌드를 멈춘다
- 꼭 바꿔야 하면 옛 주소에서 리다이렉트를 남긴다

## 다음

1. ~~색과 타입 스케일 확정~~ · ~~사이트맵 확정~~ · ~~Next 스캐폴드~~
2. 화면 보고 조판 다듬기
3. 사이드노트·접이식·무채색·다크모드를 남길지 정하기
