...
/                    홈. 목록이 아니라 관점으로 연다
/work                프로젝트 목록
/work/digestube      RAG 상세. 이 사이트에서 밀도가 가장 높은 자리
/work/vizuden        도메인 사서 운영 중인 서비스
/about               철학과 이력, 측정한 숫자
/writing             기술 블로그. 수료 요건 2편이 여기로
/writing/[slug]

/llms.txt            에이전트용 사이트 지도
/okf/                Open Knowledge Format 번들
```
...
| 역할 | 서체 | 쓰는 곳 |
| --- | --- | --- |
| 디스플레이 | 시스템 세리프 (`ui-serif, Georgia, 'Times New Roman'`) | 페이지당 한 번. 트래킹 −0.03em |
| 본문 · UI | Pretendard Variable | 본문, 라벨, 내비 |
| 데이터 · 코드 | IBM Plex Mono | 수치, 경로, 코드 블록 |

시스템 세리프는 0KB다. 한글 디스플레이는 세리프 대신 Pretendard로 떨어뜨린다.
한글 명조 시스템 서체가 플랫폼마다 달라 조판이 흔들린다.
...
- 팔레트와 타입 스케일 초안을 실제 화면에서 검증. 지금은 숫자만 있다
